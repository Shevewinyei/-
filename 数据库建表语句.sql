...
(
   com_id               int not null,
   ord_ord_id           int not null,
   ord_id               int,
   count                int,
   price                numeric(17,2),
   discount             numeric(2,2),
   primary key (com_id, ord_ord_id)
);
...
alter table FD_com_connect add constraint FK_FD_com_connect foreign key (FD_id)
...
alter table Relationship_2 add constraint FK_Relationship_2 foreign key (usr_id)
      references user (usr_id) on delete restrict on update restrict;

alter table Relationship_4 add constraint FK_Relationship_4 foreign key (cou_id)
...
alter table Relationship_7 add constraint FK_Relationship_7 foreign key (purchase_id)
...
alter table com_menu add constraint FK_com_menu foreign key (men_id)
...
alter table order_content add constraint FK_order_content foreign key (ord_ord_id)
      references "order" (ord_id) on delete restrict on update restrict;
